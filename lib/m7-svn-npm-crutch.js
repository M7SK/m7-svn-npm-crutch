...
logok = function (msg) {
	console.log('m7-svn-crutch '.green + msg);
};

logerr = function (msg) {
	console.log('m7-svn-crutch '.red + msg);
};
...
svnExport = function (dep) {
	return function (callback) {
		logok('Exporting ' + dep + ' from svn...');
		var cmd = cp.spawn('svn', ['export', deps[dep], dep], {
			stdio: 'inherit',
			cwd: rootDir + '/node_modules/'
		});

		cmd.on('exit', function (code) {
			var errMsg = 0 !== code ? 'svn export failed' : null;
			callback(errMsg);
		});
	};
};
